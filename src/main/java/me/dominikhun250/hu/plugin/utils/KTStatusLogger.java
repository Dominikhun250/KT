package me.dominikhun250.hu.plugin.utils;
...
public class KTStatusLogger {
    ...
    private void logWithStats(String title, int serversUsing, int playersUsing) {
        ...
        logger.info(DARK_GRAY + "║" + repeat(" ", (WIDTH - title.length()) / 2) + CYAN + title + DARK_GRAY + repeat(" ", WIDTH - ((WIDTH - title.length()) / 2) - title.length()) + "║" + RESET);
        logger.info(DARK_GRAY + "╠" + repeat("═", WIDTH) + "╣" + RESET);

        logger.info(CYAN + "  Version: " + LIGHT_GRAY + pluginName + " " + version + RESET);
        logger.info(CYAN + "  Author: " + LIGHT_GRAY + author + RESET);
        logger.info(CYAN + "  Effects loaded: " + RESET + effectsCount + RESET);
        logger.info(CYAN + "  Database: " + (dbOpen ? GREEN + "Connected" : RED + "Closed") + RESET);
        logger.info(CYAN + "  Economy: " + (econActive ? GREEN + "Enabled" : RED + "Disabled") + " " + RESET + "(" + econType + ")" + RESET);
        logger.info(CYAN + "  Resource Pack: " + (hasResourcePack ? RESET + resourcePackUrl : RED + "Not configured") + RESET);

        logger.info(CYAN + "  Servers using KT: " + RESET + (serversUsing >= 0 ? serversUsing : "Unknown") + RESET);
        logger.info(CYAN + "  Players using KT: " + RESET + (playersUsing >= 0 ? playersUsing : "Unknown") + RESET);

        logger.info(CYAN + "  SpigotMC: " + RESET + SPIGOT_LINK + RESET);
        logger.info(CYAN + "  GitHub: " + RESET + GITHUB_LINK + RESET);

        logger.info(DARK_GRAY + "╚" + repeat("═", WIDTH) + "╝" + RESET);

        logger.info(CYAN + centerText("Enjoy your game! :)", WIDTH) + RESET + "\n");
    }
